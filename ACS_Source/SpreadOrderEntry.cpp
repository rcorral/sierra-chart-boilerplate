...
SCSFExport scsf_TradingSpreadOrderEntry(SCStudyInterfaceRef sc)
{
	...
	bool UseFullContractValues = Input_UseFullContractValuesForSpreadPrices.GetYesNo();
	...
	for (int SpreadOrderDataIndex = 0; SpreadOrderDataIndex < NUM_SPREAD_ORDERS; ++SpreadOrderDataIndex	)
	{
		...
		{
			if (UseFullContractValues)
			{
				CurrentSpreadValue += (BasicSymbolData.Ask / BasicSymbolData.TickSize * max(BasicSymbolData.CurrencyValuePerTick, BasicSymbolData.TickSize)) * SpreadQuantities[SpreadOrderDataIndex];
			}
			else
				CurrentSpreadValue += BasicSymbolData.Ask * SpreadQuantities[SpreadOrderDataIndex];
		...
		else if (SpreadSides[SpreadOrderDataIndex] == 1)//Sell
		{
			if (UseFullContractValues)
			{
				CurrentSpreadValue -= (BasicSymbolData.Bid / BasicSymbolData.TickSize * max(BasicSymbolData.CurrencyValuePerTick, BasicSymbolData.TickSize)) * SpreadQuantities[SpreadOrderDataIndex];
			}
			else
				CurrentSpreadValue -= BasicSymbolData.Bid * SpreadQuantities[SpreadOrderDataIndex];
	...
	double SpreadPricesToCompareTo[2] = { 0 };
