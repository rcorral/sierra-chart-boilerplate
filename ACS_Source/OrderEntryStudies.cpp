...
{
	SCSubgraphRef OrderLine = sc.Subgraph[0];
	SCInputRef NumberOfBarsBack = sc.Input[3];
	SCInputRef BidAskQuantityThreshold = sc.Input[4];
	SCInputRef DebuggingOutput = sc.Input[5];
	...
	if (sc.SetDefaults)
	{
		...
		BidAskQuantityThreshold.Name = "Bid Ask Quantity Threshold";
		BidAskQuantityThreshold.SetInt(100);

		DebuggingOutput.Name = "Debugging Output";
		DebuggingOutput.SetYesNo(false);
	...
	if (sc.MenuEventID != 0 && sc.MenuEventID == r_SetBuyOrderPriceMenuID)	
	{
		...
		OrderLine.Name.Format("Buy Stop (%d)", BidAskQuantityThreshold.GetInt());
	...
	else if (sc.MenuEventID != 0 && sc.MenuEventID == r_SetSellOrderPriceMenuID)	
	{
		...
		OrderLine.Name.Format("Sell Stop (%d)", BidAskQuantityThreshold.GetInt());
	...
	{
		if (DebuggingOutput.GetYesNo() && r_LastDebugMessageType != 1)
		{
	...
	if (r_OrderType == 0)
	{
		if (DebuggingOutput.GetYesNo() && r_LastDebugMessageType != 2)
		{
	...
	if (r_StopOrderActive != 0 && r_OrderType == -1  && sc.Bid != 0.0)
	{
		...
		if ((sc.FormattedEvaluate( CurrentPriceToOrderPriceDifference, sc.BaseGraphValueFormat,  EQUAL_OPERATOR, 0.0 , sc.BaseGraphValueFormat)
			&& sc.BidSize <= BidAskQuantityThreshold.GetInt())
		...
		{
			if (DebuggingOutput.GetYesNo() && r_OrderEvaluationCount % 10 == 0)
			{
				SCString MessageText;
				MessageText.Format( "Sell stop order not triggered. Bid=%f, OrderPrice=%f, BidQuantity=%d, BidQuantityThreshold=%d.", sc.Bid, r_OrderPrice, sc.BidSize , BidAskQuantityThreshold.GetInt());
	...
	if (r_StopOrderActive != 0 && r_OrderType == 1   && sc.Ask != 0.0)
	{
		...
		if ((sc.FormattedEvaluate( CurrentPriceToOrderPriceDifference, sc.BaseGraphValueFormat,  EQUAL_OPERATOR, 0.0 , sc.BaseGraphValueFormat)
			&& sc.AskSize <= BidAskQuantityThreshold.GetInt())
		...
		else
		{
			...
			{
				SCString MessageText;
				MessageText.Format( "Buy stop order not triggered. Ask=%f, OrderPrice=%f, AskQuantity=%d, AskQuantityThreshold=%d.", sc.Ask, r_OrderPrice, sc.AskSize , BidAskQuantityThreshold.GetInt());
				sc.AddMessageToLog(MessageText, 0);
			}
		}
	}
...
SCSFExport scsf_TradingTriggeredStopOrderEntry(SCStudyInterfaceRef sc)
{
	...
	SCInputRef NumberOfBarsBack = sc.Input[0];
	SCInputRef AutoSetStopPrice = sc.Input[1];
	SCInputRef StopPriceOffsetInTicks = sc.Input[2];
	...
	if (sc.SetDefaults)
	{
		...
		StopPriceOffsetInTicks.Name = "Stop Price Offset from Trigger Price in Ticks";
		StopPriceOffsetInTicks.SetInt(4);
		...
		TriggerLine.Name = "Stop Order Trigger";
	...
	if (sc.MenuEventID != 0 && sc.MenuEventID == r_SetTriggerPriceMenuID)	
	{
		...
		{
			r_OrderPrice = sc.ChartTradingOrderPrice + StopPriceOffsetInTicks.GetInt() * sc.TickSize;
			r_OrderType = 1;
		}
		else  // Sell
		{
	...
	if (r_OrderType == -1)
		OrderLine.Name = "Sell Stop Order";
	else if(r_OrderType == 1)
		OrderLine.Name = "Buy Stop Order";
