...
SCSFExport scsf_TradingExampleWithAttachedOrders(SCStudyInterfaceRef sc)
{
	...
	// Calculate the moving average
	sc.SimpleMovAvg(sc.Close, SimpMovAvgSubgraph, sc.Index, 10);
	...
	// Buy when the last price crosses the moving average from below.
	if (sc.CrossOver(sc.Close, SimpMovAvgSubgraph) == CROSS_FROM_BOTTOM && sc.GetBarHasClosedStatus() == BHCS_BAR_HAS_CLOSED)
	{
	...
	// Sell when the last price crosses the moving average from above.
	else if (sc.CrossOver(sc.Close, SimpMovAvgSubgraph) == CROSS_FROM_TOP && sc.GetBarHasClosedStatus() == BHCS_BAR_HAS_CLOSED)
	{
...
SCSFExport scsf_TradingExampleUsingReversals(SCStudyInterfaceRef sc)
{
	...
	// Calculate the moving average
	sc.SimpleMovAvg(BarLastArray, SimpMovAvgSubgraph, sc.Index, 3);
...
SCSFExport scsf_TradingSystemStudySubgraphCrossover(SCStudyInterfaceRef sc)
{
	...
	if (sc.SetDefaults)
	{
		...
		BuyEntry.Name = "Buy";
		BuyEntry.DrawStyle = DRAWSTYLE_POINT_ON_HIGH;
		BuyEntry.LineWidth = 4;

		SellEntry.Name = "Sell";
		SellEntry.DrawStyle = DRAWSTYLE_POINT_ON_LOW;
		SellEntry.LineWidth = 4;
...
SCSFExport scsf_ACSILTradingTest2(SCStudyInterfaceRef sc)
{
	...
	//LimitTemplate.OrderQuantity =1000;
	//LimitTemplate.Price1 = sc.Close[sc.Index] - 10;
	//int ReturnValue = sc.BuyOrder(LimitTemplate);
