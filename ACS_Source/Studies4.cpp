...
SCSFExport scsf_WoodieCCITrend2(SCStudyInterfaceRef sc)
{
	...
	if(sc.SetDefaults)
	{
		...
		CCIInputData.SetInputDataIndex(SC_HLC_AVG);
...
SCSFExport scsf_CandleStickPatternsFinder(SCStudyInterfaceRef sc)
{
	...
	SCInputRef DisplayAboveCandle = sc.Input[13];
	...
	if (sc.SetDefaults)
	{
		...
		DisplayAboveCandle.Name= "Display Above Candles";
		DisplayAboveCandle.SetYesNo(false);
	...
	if (PatternCodesToDisplay && sc.Index < sc.ArraySize -1)
	{
		...
		if (DisplayAboveCandle.GetYesNo())
		{
			Tool.BeginValue = sc.High[sc.Index] + ( (sc.High[sc.Index] - sc.Low[sc.Index])  * Distance.GetInt() * 0.01f);
...
SCSFExport scsf_WoodiesZLR(SCStudyInterfaceRef sc)
{
	...
	for (int index = sc.UpdateStartIndex; index < sc.ArraySize; index++)
	{
		...
		if(ZLRLongTrade)
		{
			...
			if (AlertNumber.GetAlertSoundNumber() > 0)
				sc.SetAlert(AlertNumber.GetAlertSoundNumber() - 1, "Long Trade" );
		}
		...
		if(ZLRShortTrade)
		{
			...
			if (AlertNumber.GetAlertSoundNumber() > 0)
				sc.SetAlert(AlertNumber.GetAlertSoundNumber() - 1, "Short Trade");
		}
	}
}
...
SCSFExport scsf_StochBands(SCStudyInterfaceRef sc)
{
	...
	if (sc.SetDefaults)
	{
		...
		InputDataType.SetInputDataIndex(SC_HL_AVG);
...
SCSFExport scsf_AutomaticTrendlines(SCStudyInterfaceRef sc)
{
	...
	if (sc.SetDefaults)
	{
		...
		LengthInput.SetIntLimits(2,MAX_STUDY_LENGTH);
	...
	else
	{
		...
		if(NewBarsBeforeAdjust.GetInt() > 0)
		{
			...
			if ((BarIndexDifference < NewBarsBeforeAdjust.GetInt()))
			{
				...
				if(UpLineIndex2 < LastExtendedIndex || DownLineIndex2 < LastExtendedIndex)
				{
					if(UpLineIndex2 > 0) 
					{
						for(int Index=UpLineIndex1; Index<=LastExtendedIndex; Index++ )
							UpTrendline[Index]	= (float)(UpLine_A*Index + UpLine_B);
					...
					if(DownLineIndex2 > 0) 
					{
						for(int Index=DownLineIndex1; Index<=LastExtendedIndex; Index++ )
							DownTrendline[Index]	= (float)(DownLine_A*Index + DownLine_B);
...
inline int RenkoFillHigh(SCStudyInterfaceRef sc, int32_t BaseGraphIndex, int32_t& RenkoIndex, float price, bool swap)
{
	...
	float& brickSize   = sc.GetPersistentFloat(1);
	float& renkoHigh   = sc.GetPersistentFloat(2);
	float& renkoLow    = sc.GetPersistentFloat(3);
	int&   barComplete = sc.GetPersistentInt(2);
...
inline int RenkoFillLow(SCStudyInterfaceRef sc, int32_t BaseGraphIndex, int32_t& RenkoIndex, float price, bool swap)
{
	SCSubgraphRef Open      = sc.Subgraph[SC_OPEN];
	SCSubgraphRef High      = sc.Subgraph[SC_HIGH];
	SCSubgraphRef Low       = sc.Subgraph[SC_LOW];
...
inline int RenkoFill(SCStudyInterfaceRef sc, int32_t BaseGraphIndex, int32_t& RenkoIndex, float price, bool swap)
{
	int numBarsAdded = 0;
...
SCSFExport scsf_RenkoChartEnhanced(SCStudyInterfaceRef sc)
{
	...
	SCFloatArrayRef iNumTrades     = sc.BaseDataIn[SC_NUM_TRADES];
	...
	SCFloatArrayRef iUpDownTickDiffMax = sc.BaseDataIn[SC_ASKBID_NUM_TRADES_DIFF_HIGH];
	SCFloatArrayRef iUpDownTickDiffMin = sc.BaseDataIn[SC_ASKBID_NUM_TRADES_DIFF_LOW];
	...
	SCSubgraphRef UpDownTickDiffMax = sc.Subgraph[SC_ASKBID_NUM_TRADES_DIFF_HIGH];
	SCSubgraphRef UpDownTickDiffMin = sc.Subgraph[SC_ASKBID_NUM_TRADES_DIFF_LOW];
	...
	{
		// Set the defaults
		sc.GraphName = "Renko Chart Enhanced (unsupported)";
		...
		sc.GraphDrawType = GDT_RENKO_BRICK_WITH_WICKS;
		sc.AutoLoop = 0;
		sc.ValueFormat = VALUEFORMAT_INHERITED;
...
// UpDownTickDiffMin.DrawStyle = DRAWSTYLE_IGNORE;
	...
	float& brickSize   = sc.GetPersistentFloat(1);  // brickSize in terms of price
	float& renkoHigh   = sc.GetPersistentFloat(2);
	float& renkoLow    = sc.GetPersistentFloat(3);
	...
	float& lastHigh        = sc.GetPersistentFloat(4);
	float& lastLow         = sc.GetPersistentFloat(5);
	float& lastVolume      = sc.GetPersistentFloat(6);
	float& lastNumTrades   = sc.GetPersistentFloat(7);
	float& lastBidVol      = sc.GetPersistentFloat(8);
	float& lastAskVol      = sc.GetPersistentFloat(9);
	float& lastUpTickVol   = sc.GetPersistentFloat(10);
	float& lastDownTickVol = sc.GetPersistentFloat(11);
	float& lastBidTrades   = sc.GetPersistentFloat(12);
	float& lastAskTrades   = sc.GetPersistentFloat(13);
	...
	if (sc.UpdateStartIndex == 0)
	{
		...
		sc.GraphName.Format("%s Renko - BarSize: %i", sc.GetStudyName(0).GetChars(), brickSizeTicks.GetInt());
	...
	int32_t RenkoIndex = sc.OutArraySize - 1;

	for (int32_t BaseGraphIndex = sc.UpdateStartIndex; BaseGraphIndex < sc.ArraySize; BaseGraphIndex++)
	{
		...
		else
		{
			...
			for (int32_t VolumeUpdateIndex = RenkoIndex - numBarsAdded + 1; VolumeUpdateIndex < RenkoIndex; VolumeUpdateIndex++)
			{
