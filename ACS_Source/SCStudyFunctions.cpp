...
SCFloatArrayRef WeightedMovingAverage_S(SCFloatArrayRef In, SCFloatArrayRef Out, int Index, int Length)
{
	...
	{
		Sum += In[PriorIndex] * (float)(CurrentWeight);
		CurrentWeight--;
	}
...
)
{
	...
	else // Use Daily Chart
	{
		...
		else
		{
			...
			if (DailyChartDateTimes[FirstIndexOfReferenceDay] == IntradayChartDate
...
{
	float OpenVal = BaseDataIn[SC_OPEN][Index];
	float HighVal = BaseDataIn[SC_HIGH][Index];
	float LowVal = BaseDataIn[SC_LOW][Index];
	float LastVal = BaseDataIn[SC_LAST][Index];
...
SCFloatArrayRef HurstExponentNew_S(SCFloatArrayRef In, SCFloatArrayRef Out, int Index, int LengthIndex)
{
	...
	for (int IterationIndex = 0; IterationIndex < ArrayIterations; IterationIndex++)
	{
		...
		for (int SubsetIndex = 0; SubsetIndex < CurrentNumberOfSubsets; SubsetIndex++)
		{
			...
			float Mean = Sum / CurrentSubsetLength;
			...
			SeriesIndex = 0;
			for (int SeriesIndex = 0; SeriesIndex < CurrentSubsetLength; SeriesIndex++)
			{
				float Sum = 0;
				for (int MeanAdjustedSeriesIndex = 0; MeanAdjustedSeriesIndex < SeriesIndex + 1; MeanAdjustedSeriesIndex++)
				{ 
					Sum += MeanAdjustedSeries[MeanAdjustedSeriesIndex];
				}

				CumulativeVariateSeries[SeriesIndex] = Sum;
